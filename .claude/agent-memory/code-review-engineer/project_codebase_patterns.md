...
## OAuth / Token Patterns (established in TASK-007 google-auth, reviewed 2026-06-15)
...
- **`SessionAuthFilter`** promotes the custom `adminId` session attribute to a Spring Security `UsernamePasswordAuthenticationToken` before `AnonymousAuthenticationFilter`. `/calendar/google/connect` is therefore protected at two independent layers: Spring Security filter chain (401 before controller runs) and `SessionUtils.requireAdminId()` inside the controller. `/calendar/google/callback` remains in `permitAll()` — it is secured by the HMAC-signed state parameter instead of a session.
...
- **Refresh token null guard**: `handleCallback` throws `IllegalStateException` when Google omits the refresh token (e.g., reconnect without revoking). This prevents silently overwriting a valid stored refresh token with an empty encrypted string.
- **`JdbcCalendarAccountRepositoryTest`** uses `@MockBean GoogleOAuthService` and `@MockBean GoogleTokenRefresher` to prevent Spring from initializing those beans (which require real Google credentials) during repository integration tests.
